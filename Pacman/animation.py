...
def load_images():
    ...
    # Load all the images
    # Pacman's images
    pacman_normal = pygame.image.load('Pacman/Images/Pacman/pacman_normal.png')
    pacman_movement = {pacman.UP_dir: pygame.image.load('Pacman/Images/Pacman/pacman_U.png'),
                       pacman.DOWN_dir: pygame.image.load('Pacman/Images/Pacman/pacman_D.png'),
                       pacman.LEFT_dir: pygame.image.load('Pacman/Images/Pacman/pacman_L.png'),
                       pacman.RIGHT_dir: pygame.image.load('Pacman/Images/Pacman/pacman_R.png'),
    ...
    for i in range(3, 8):
        pacman_death.append(pygame.image.load('Pacman/Images/Pacman/pacman_die_{}.png'.format(i)))
    # Ghosts images
    ...
    for ghost in ['Blinky', 'Pinky', 'Inky', 'Clyde']:
        images = []
        for dirn in ['U', 'L', 'D', 'R']:
            path = 'Pacman/Images/{}/{}_{}.png'.format(ghost, ghost, dirn)
            image = pygame.image.load(path)
            images.append(image)
        ghost_images[ghost] = images

    frightened_ghost_1 = pygame.image.load('Pacman/Images/Frightened_1.png')
    frightened_ghost_2 = pygame.image.load('Pacman/Images/Frightened_2.png')


def animate_ghost(ghost, current_time, start_time):
    if ghost.state != 'FRIGHTENED':
        # Use the normal images
    ...
    if player.alive:
        ...
        if current_time - start_time < 3000:
            ...
            try:
                ...
                return pacman_death[-2]  # Transparent image
